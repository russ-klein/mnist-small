...
#define USER_MEMORY_MODULE "ccs_ram_w_handshake"
  ...
  void updateCounts()
  {
    if ((rst_ph && rstn_int.read() == SC_LOGIC_1) || (!rst_ph && rstn_int.read() == SC_LOGIC_0))  {
      ...
      if (readReqPending) {
        ...
        } else if (m_re.read() != SC_LOGIC_1) {
          readReqPending = false; // have to hold s_re high the whole time until s_rrdy
        }
...
#ifdef DBG_CB_R
      cout << "ReadCounts: readReqPending=" << readReqPending << " T=" << sc_time_stamp() << std::endl;
#endif
    }
