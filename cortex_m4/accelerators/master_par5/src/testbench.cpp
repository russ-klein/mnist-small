...
hw_cat_type read_cat_memory_as_fixed(int offset)
{
    ...
    return get_line_word(weight_memory[major], minor);
...
void write_cat_memory_as_fixed(int offset, hw_cat_type value)
{
    ...
    set_line_word(line, minor, value);
              ...
              int num_output_images)
{
    ...
    for (i=0; i<(num_output_images + (STRIDE-1))/STRIDE; i++) {
        ...
        for (p=0; p<STRIDE; p++) {
            write_cat_memory_as_fixed(output_offset + i * STRIDE + p, get_line_word(line,p));
        }
    }
    done.read();
}
    ...
    for (o=0; o<num_output_images; o++) {
        for (i=0; o<num_input_images; i++) {
            for (n=0; n<(height*width + (STRIDE-1))/STRIDE; n++) {
                ...
                IMAGE_IN_CHANNEL = weight_memory[image_offset + height*width*i + n];
            }
            for (n=0; n<(filter_height*filter_width + (STRIDE-1))/STRIDE; n++) {
                while (!FILTER_IN_CHANNEL_READY);
                FILTER_IN_CHANNEL = weight_memory[weight_offset + filter_height*filter_width*i + n];
            }
        }
    }
              ...
              int num_output_images)
{
    ...
    for (int i=0; i<num_input_images;i++) {
        for (int c=0; c<(((image_height*image_width) + (STRIDE-1))/STRIDE); c++) {
            ...
            DENSE_WEIGHTS_CHANNEL = memory_base[weight_offset + image_height*image_width*i + c];
        }
    }
...
sw_cat_type get_cat_value(cat_memory_type *m, int index)
{
    int major, minor;
    int n;
    float r;
...
void set_cat_value(cat_memory_type *m, int index, sw_cat_type value)
{
    int major, minor;
    int mask;
    int n;
...
void hw_inference(unsigned char *input_image, host_type *probabilities)
{
    ...
    hw_infer(top_of_weights + hw_offset, probabilities); // hint: weights getting clobbered
