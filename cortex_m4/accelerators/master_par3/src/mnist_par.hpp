...
typedef ac_int<32 * BUS_WIDTH, false> raw_memory_line;

//#ifdef SLAVE
typedef ac_channel<raw_memory_line> pipe_type;

//typedef hw_cat_type   image_type[HEIGHT][WIDTH];
//typedef hw_cat_type   filter_type[FILTER_HEIGHT][FILTER_WIDTH];
//#else
...
// typedef image_slice_type     image_type[STRIDE];
// typedef filter_slice_type    filter_type[STRIDE];
//#endif

// accessors for memory and bus line buffers

void set_line_word(raw_bus_type &line, index_type index, hw_cat_type word)
{
    // isolate all the slicing crap
    line.set_slc(index * WORD_SIZE, word.slc<WORD_SIZE>(0));
}

hw_cat_type get_line_word(raw_bus_type &line, index_type index)
{
    hw_cat_type t;
...
/*
void set_line_word(raw_memory_line &line, index_type index, hw_cat_type word)
{
    line.set_slc(index * WORD_SIZE, word.slc<WORD_SIZE>(0));
}

hw_cat_type get_line_word(raw_memory_line &line, index_type index)
{
    hw_cat_type t;
...
void print_image(raw_bus_type *m, int addr)
{
    ...
    for (r=0; r<HEIGHT; r++) {
        for (c=0; c<WIDTH; c++) {
            ...
            value = get_line_word(*(m + offset), index);
            if ((-0.001 < value) && (value < 0.001)) printf("   -  ");
            else printf("%5.3f ", value.to_double());
        }
        printf("\n");
...
#pragma hls_unroll
        for (p=0; p<stride; p++) {
            if ((min <= (mem_addr + p)) && ((mem_addr + p) < max)) {
                data[d_offset + p - diff] = get_line_word(t, p); // test me
...
#pragma hls_unroll
        for (p=0; p<stride; p++) {
            buffer[p] = ((min <= (mem_addr + p)) && ((mem_addr + p) < max)) ? data[d_offset - diff + p] : get_line_word(t, p);
            set_line_word(t, p, buffer[p]);
...
void load_dense_features_slave(pipe_type *dense_in_channel, index_type num_input_elements, raw_memory_line *input_image)
{
   ...
   #pragma hls_pipeline_init_interval 1
    for (i=0; i<message_count; i++) {
        ...
        if (chatty) {
            for (p=0; p<STRIDE; p++) {
                printf("word[%d]: %5.3f \n", (i * STRIDE + p).to_int(), get_line_word(input_image[i], p).to_double());
            }
        }
    }
}
...
hw_cat_type load_weights_and_multiply_slave(pipe_type *dense_in_channel, index_type num_input_elements, raw_memory_line *input_image)
{
   ...
   #pragma hls_pipeline_init_interval 1
    for (n=0; n<message_count; n++) {
       ...
       #pragma hls_unroll
        for (p=0; p<STRIDE; p++) {
            weight_value = get_line_word(weight_line, p);
            image_value = get_line_word(feature_line, p);
...
void write_dense_out_slave(pipe_type *dense_out_channel, raw_memory_line *dense_out, index_type num_output_elements)
{
   ...
   #pragma hls_pipeline_init_interval 1
    for (o=0; o<message_count; o++) {
       #pragma hls_unroll
        for (p=0; p<STRIDE; p++) {
            value = ((count + p) < num_output_elements) ? get_line_word(dense_out[o], p) : 0.0;
            set_line_word(line, p, value);
        }
        count += stride;
...
#pragma hls_unroll
    for (p=0; p<STRIDE; p++) {
        ...
        t = (bus_word_valid) ? get_line_word(input_buffer, p) : 0.0;
    ...
    index_type    bus_alignment_offset)
{
   ...
   #pragma hls_unroll
    for (p=0; p<STRIDE; p++) {
        ...
        t = (bus_word_valid) ? array_buffer[p - bus_alignment_offset] : get_line_word(output_buffer, p);
        set_line_word(output_buffer, p, t);
        if (bus_word_valid) {
...
void store_into_system_memory(raw_memory_line *output_array, index_type array_offset, index_type size, raw_bus_type *memory_in, raw_bus_type *memory_out, index_type offset)
{
    raw_bus_type output_buffer;
    ...
    while (count<size) {
        ...
        if (first && ((size < bus_line_width) || (bus_min != bus_line_start))) {
            output_buffer= memory_in[bus_write_address];
            ...
            output_buffer = memory_in[bus_write_address+1];
        ...
        memory_out[bus_write_address] = output_buffer;
...
#else // MASTER
                 raw_bus_type memory_in    [0x100000],
                 raw_bus_type memory_out    [0x100000],
                 ...
                 index_type num_output_images)
{
    ...
    if (convolve) {
        ...
        for (i=0; i<num_input_images; i++) {
            ...
            load_from_system_memory(memory_in, image_pointer, image_size, input_image_mem[i], 0);
        ...
        for (o=0; o<num_output_images; o++) {
            for (i=0; i<num_input_images; i++) {
                ...
                load_from_system_memory(memory_in, weight_pointer, filter_size, filter_mem, 0);
                perform_convolution(input_image_mem[i], filter_mem, output_image_pr_mem, i);
                weight_pointer += filter_size;
            }
            perform_relu(relu, output_image_mem, output_image_pr_mem);
            store_into_system_memory(output_image_mem, 0, HEIGHT*WIDTH, memory_in, memory_out, output_pointer);
...
#else
        load_from_system_memory(memory_in, image_offset, num_input_images, dense_in_mem, 0);
        ...
        while (count<num_output_images) {
            for (p=0; p<stride; p++) {
                if ((out_pointer + p) < num_output_images) {
                    dense_out_buffer[p] = load_weights_and_multiply_master(memory_in, weight_offset + num_input_images * count, num_input_images, dense_in_mem);
                    count++;
                } else {
        ...
        store_into_system_memory(dense_out_mem, 0, num_output_images, memory_in, memory_out, output_offset);
#endif
    }
    done.write(1);
}
