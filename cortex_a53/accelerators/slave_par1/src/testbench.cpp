               ...
               int relu)
{
    ...
    NUM_INPUT_IMAGES_REG = num_input_images;
    NUM_OUTPUT_IMAGES_REG = num_output_images;
    CONVOLVE_REG = 1;
    RELU_REG = 1;
    FULLY_CONNECTED_REG = 0;
    ...
    for (o=0; o<num_output_images; o++) {
        for (i=0; o<num_input_images; i++) {
            for (n=0; n<(height*width + (STRIDE-1))/STRIDE; n++) {
                while (!IMAGE_CHANNEL_IN_READY_REG);
                IMAGE_IN_CHANNEL_REG = memory_base[image_offset + height*width*i + n];
            }
            for (n=0; n<(filter_height*filter_width + (STRIDE-1))/STRIDE; n++) {
                while (!FILTER_IN_CHANNEL_READY_REG);
                FILTER_IN_CHANNEL_REG = memory_base[weight_offset + filter_height*filter_width*i + n];
            }
        }
        for (n=0; n<(height*width + (STRIDE-1))/STRIDE; n++) {
            while (!IMAGE_OUT_CHANNEL_VALID_REG);
            memory_base[output_offset + height*width*o + n] = IMAGE_OUT_CHANNEL_REG;
        }
    }
              ...
              int num_output_images)
{
    ...
    for (int i=0; i<num_input_images;i++) {
        for (int c=0; c<(((image_height*image_width) + (STRIDE-1))/STRIDE); c++) {
            while (!DENSE_IN_CHANNEL_READY_REG);
            DENSE_IN_CHANNEL_REG = memory_base[image_offset + image_height*image_width*i + c];
        }
    }
    
    for (int i=0; i<num_input_images;i++) {
        for (int c=0; c<(((image_height*image_width) + (STRIDE-1))/STRIDE); c++) {
            while (!DENSE_WEIGHT_CHANNEL_READY_REG);
            DENSE_WEIGHT_CHANNEL_REG = memory_base[weight_offset + image_height*image_width*i + c];
        }
    }
    ...
    for (o=0; o<(num_output_images + (STRIDE-1))/STRIDE; o++) {
        while (!DENSE_OUT_CHANNEL_DONE_REG);
        memory_base[output_offset + o] = DENSE_OUT_CHANNEL_REG;
    }
}
               ...
               int relu)
{
    ...
    for (i=0; i<num_input_images; i++) {
        for (n=0; n<(height*width + (STRIDE-1))/STRIDE; n++) {
            ...
            while (!IMAGE_IN_CHANNEL_READY_REG);
            IMAGE_IN_CHANNEL_REG = outgoing_word;
        }
    }
    for (o=0; o<num_output_images; o++) {
        for (i=0; i<num_input_images; i++) {
            for (n=0; n<(filter_height*filter_width + (STRIDE-1))/STRIDE; n++) {
                ...
                while (!FILTER_IN_CHANNEL_READY_REG);
                FILTER_IN_CHANNEL_REG = outgoing_word;
            }
        }

        for (n=0; n<(height*width + (STRIDE-1))/STRIDE; n++) {
            while (!IMAGE_OUT_CHANNEL_VALID_REG);
            incoming_word = IMAGE_OUT_CHANNEL_REG;
            for (p=0; p<STRIDE; p++) {
                set_cat_value_as_fixed(output_offset + height*width*o + n*STRIDE + p, (incoming_word >> (WORD_SIZE * p)) & ((1 << WORD_SIZE) -1));
            }
            //weight_memory[output_offset + height*width*o + n] = IMAGE_OUT_CHANNEL_REG;
        }
    }
              ...
              int num_output_images)
{
    ...
    for (int i=0; i<(num_input_images + (STRIDE-1))/STRIDE; i++) {
        ...
        while (!DENSE_IN_CHANNEL_READY_REG);
        DENSE_IN_CHANNEL_REG = outgoing_word;
    }
    ...
    for (int o=0; o<num_output_images; o++) {
        for (int i=0; i<(num_input_images + (STRIDE-1))/STRIDE; i++) {
            ...
            while (!DENSE_WEIGHTS_CHANNEL_READY_REG);
            DENSE_WEIGHTS_CHANNEL_REG = outgoing_word;
        }
    }
    
    for (o=0; o<(num_output_images + (STRIDE-1))/STRIDE; o++) {
        while (!DENSE_OUT_CHANNEL_VALID_REG);
        incoming_word = DENSE_OUT_CHANNEL_REG;
        for (p=0; p<STRIDE; p++) {
            set_cat_value_as_fixed(output_offset + o*STRIDE +p, (incoming_word >> (WORD_SIZE * p)) & ((1 << WORD_SIZE) -1));
        }
        //weight_memory[output_offset + o] = DENSE_OUT_CHANNEL_REG;
    }
               ...
               int relu)
{
    ...
    IMAGE_OFFSET_REG = image_offset;
    WEIGHT_OFFSET_REG = weight_offset;
    OUTPUT_OFFSET_REG = output_offset;
    NUM_INPUT_IMAGES_REG = num_input_images;
    NUM_OUTPUT_IMAGES_REG = num_output_images;
...
sw_cat_type get_cat_value(bus_type *m, int index)
{
    ...
    //major = index/STRIDE;
    //minor = index%STRIDE;
    major = index/(32/WORD_SIZE);
    minor = index%(32/WORD_SIZE);
               ...
               int relu)
{
    ...
    sw_cat_type sum;
    sw_cat_type n;
    sw_cat_type image_value;
    sw_cat_type weight_value;
    int image_index;
    int weight_index;
    int output_index;
...
#ifdef ARM
                    if (chatty) printf("sum[%d] = %5.2f \n", output_index, sum);
#else 
                    if (chatty) printf("sum[%d] = %5.2f \n", output_index, sum.to_double());
#endif
                }
            }
        }
...
void conv_test()
{
    ...
    copy_to_cat(0, null_filter, 9);
    copy_to_cat(9, array_in, 28*28);
    
    conv2d_hw(18, 0, 18 + 28*28, 1, 2, 28, 28, 3, 3, 0);

    copy_from_cat(array_out, 9 + 28 * 28, 28*28);

    for (i=0; i<28; i++) {
       for (j=0; j<28; j++) {
          printf("%3d ", (int) array_out[28*28+i*28+j]);
       }
       printf("\n");
    }
